...
def home(request):
    ...
    if len(data) == 0:
        ...
        try:
            active_cam = models.Camera_Settings.objects.get(
                id=request.session['home_cam_num'])
        except:
    ...
    if request.method == "POST":
        if request.POST['command'] == 'change_cam':
            ...
            unknown_enter, female_enter, male_enter, out, inside = MC.get_variables(
                request.session['home_cam_num'])

            today_data = models.Counted_Instances.objects.filter(
                camera_id=request.session['home_cam_num'])

            total_male_enter = [int(model.male_entries) for model in today_data if str(
                model.timestamp).split('-')[0] == datetime.now().strftime('%D')]
            total_male_enter = sum(total_male_enter[:-1]) + male_enter

            total_female_enter = [int(model.female_entries) for model in today_data if str(
                model.timestamp).split('-')[0] == datetime.now().strftime('%D')]
            total_female_enter = sum(total_female_enter[:-1]) + female_enter

            total_unknown_enter = [int(model.unknown_gender_entries) for model in today_data if str(
                model.timestamp).split('-')[0] == datetime.now().strftime('%D')]
            total_unknown_enter = sum(total_unknown_enter[:-1]) + unknown_enter

            total_exit = [int(model.people_exits) for model in today_data if str(
                model.timestamp).split('-')[0] == datetime.now().strftime('%D')]
            total_exit = sum(total_exit[:-1]) + out

            inside = (total_male_enter + total_female_enter +
                      total_unknown_enter) - total_exit
            ...
            if (total_female_enter + total_male_enter) != 0:
                female_percentage = (
                    total_female_enter / (total_female_enter + total_male_enter)) * 100

                male_percentage = (
                    total_male_enter / (total_female_enter + total_male_enter)) * 100
            ...
            male_entries, female_entries, total_entries, occupancies, exits = MC.get_daily_report(
                active_cam.id, date)
            ...
            month = datetime.strptime(
                request.POST['date'], "%Y-%m").strftime("%m/%y")

            male_entries, female_entries, total_entries = MC.get_monthly_report(
                active_cam.id, month)
            ...
            start_date = datetime.strptime(
                request.POST['start_date'], "%Y-%m-%d").strftime("%m/%d/%y")
            end_date = datetime.strptime(
                request.POST['end_date'], "%Y-%m-%d").strftime("%m/%d/%y")

            male_entries, female_entries, total_entries, date_list = MC.get_date_range_report(
                active_cam.id, start_date, end_date)
            ...
            if request.POST['entry_type'] == 'all_entries':
                ...
                personnel_entries_report = MC.get_personnel_report(
                    active_cam.id, date)
                ...
                if len(personnel_entries_report) != 0:
                    ...
                    personnel_entries_report = [personnel_entries_report[idx] for idx, val in enumerate(np.array(
                        personnel_entries_report)[:, 0]) if val not in np.array(personnel_entries_report)[:, 0][:idx]]
            ...
            if request.POST['chart_type'] == 'date':
                date = datetime.strptime(
                    request.POST['date'], "%Y-%m-%d").strftime("%m/%d/%y")
                create_counting_excel(
                    request, date, request.POST['chart_type'])
            elif request.POST['chart_type'] == 'month':
                month = datetime.strptime(
                    request.POST['date'], "%Y-%m").strftime("%m/%y")
                create_counting_excel(
                    request, month, request.POST['chart_type'])
...
def create_personnel_excel(request, date):
    ...
    entries_data = models.Personnel_Entries.objects.filter(
        camera_id=active_cam.id)
    entries_data = [entries for entries in entries_data if str(
        entries.timestamp).split('-')[0] == date]
    ...
    worksheet.write(0, 0, active_cam.cam_name +
                    ' - Personnel Report - ' + d.strftime('%d %b %Y'))
...
def create_counting_excel(request, date, chart_type):
    ...
    if chart_type == 'date':
        ...
        counting_data = models.Counted_Instances.objects.filter(
            camera_id=active_cam.id)
        counting_data = [data for data in counting_data if str(
            data.timestamp).split('-')[0] == date]

        field = ["No", "Time Range", "Male", "Female",
                 "Undetected", "Total Enter", "Total Exit", "Inside"]

        workbook = Workbook(filepath)
        worksheet = workbook.add_worksheet()

        worksheet.write(0, 0, active_cam.cam_name +
                        ' - Daily Report - ' + d.strftime('%d %b %Y'))
        ...
        for row, data in enumerate(counting_data, 2):
            ...
            time_range = current_hour + ':00 - ' + \
                str(int(current_hour) + 1) + ':00'
            total_entries = int(
                data.male_entries) + int(data.female_entries) + int(data.unknown_gender_entries)
        ...
        counting_data = models.Counted_Instances.objects.filter(
            camera_id=active_cam.id)
        counting_data = [data for data in counting_data if str(data.timestamp).split('-')[0].split('/')[0] == str(
            date).split('/')[0] and str(data.timestamp).split('-')[0].split('/')[2] == str(date).split('/')[1]]
        ...
        for day in range(1, 32):
            total_male_entries = sum([int(data.male_entries) for data in counting_data if int(
                str(data.timestamp).split('-')[0].split('/')[1]) == day])
            total_female_entries = sum([int(data.female_entries) for data in counting_data if int(
                str(data.timestamp).split('-')[0].split('/')[1]) == day])
            total_unknown_entries = sum([int(data.unknown_gender_entries) for data in counting_data if int(
                str(data.timestamp).split('-')[0].split('/')[1]) == day])

            current_date = datetime.strptime(
                str(day) + '/' + date, '%d/%m/%y').strftime('%d %b %Y')
        ...
        start_date = set_date(int(start_date.strftime('%Y')), int(
            start_date.strftime('%m')), int(start_date.strftime('%d')))
        end_date = set_date(int(end_date.strftime('%Y')), int(
            end_date.strftime('%m')), int(end_date.strftime('%d')))

        date_range = int((end_date - start_date).days) + 1

        filename = 'Counting Data Report - (' + start_date.strftime(
            '%d %b %Y') + ' - ' + end_date.strftime('%d %b %Y') + ')' + '.xlsx'
        filepath = os.path.join(os.path.dirname(__file__), 'static', filename)
        ...
        worksheet.write(0, 0, active_cam.cam_name + ' - Date Range Report - (' +
        ...
        for i in range(date_range):
            ...
            total_male_entries = sum([int(data.male_entries) for data in counting_data if data.timestamp.split(
                '-')[0] == current_date.strftime("%m/%d/%y")])
            total_female_entries = sum([int(data.female_entries) for data in counting_data if data.timestamp.split(
                '-')[0] == current_date.strftime("%m/%d/%y")])
            total_unknown_entries = sum([int(data.unknown_gender_entries) for data in counting_data if data.timestamp.split(
                '-')[0] == current_date.strftime("%m/%d/%y")])
            total_entries = total_male_entries + total_female_entries + total_unknown_entries
