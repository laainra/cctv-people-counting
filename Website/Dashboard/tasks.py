...
def presence(request):
    ...
    for data in data_list:
        ...
        detected_time = datetime.strptime(data['datetime'], '%Y-%m-%d %H:%M:%S') 
        ...
        if rec_status != "Unknown":
            ...
            check_personnel_db = models.Personnel_Entries.objects.filter(personnel_id=personnel_id)
            current_date = detected_time.date()
            current_date_str = current_date.strftime('%Y-%m-%d')
            
            # Filter records for today based on date and personnel name
            today_records = [record for record in check_personnel_db if record.timestamp.date().strftime('%Y-%m-%d') == current_date_str]
            person_today = [record for record in today_records if record.personnel.name == predicted_name]
            
            if not person_today:
                # Create folder based on the date in the absence directory
                date_folder = os.path.join(PREDICTED_ABSENCE_DIR, detected_time.strftime('%Y%m%d'))
        ...
        try:
            ...
            print(f"att start: {attendance_start}, att end: {attendance_end}, leave start: {leaving_start}, leave end: {leaving_end},  ") # ini kenapa tidak dijalankan 
            ...
            print(f"has_ontime : {has_ontime}, has_late : {has_late}, has_leave : {has_leave}") # ini kenapa tidak dijalankan 
    ...
    if request.method == 'GET':
        try:
            ...
            for entry in entries:
                ...
                relative_image_path = os.path.relpath(full_image_path, start=STATIC_DIR)
